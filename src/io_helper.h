...
void save_text_file(const std::string& file_name, const SaveFunc& save,
    Args&&... args)
{
    if (file_name == "-") {
...
auto load_uncached_text_file(const std::string& file_name, const LoadFunc& load)
    -> decltype(load(std::cin))
{
    if (file_name == "-") {
        return load(std::cin);
    } else {
