...
public:
    ...
    typename std::enable_if<
        is_id_func<IDFunc>::value && std::is_convertible<typename id_func_image_type<IDFunc>::type, T>::value,
        ArrayIDFunc&>::type
    operator=(const IDFunc& o)
    {
    ...
    void fill(const T& t) { std::fill(data_, data_ + preimage_count_, t); }
...
struct ArrayIDIDFunc : public ArrayIDFunc<int> {
    ...
    ArrayIDIDFunc() noexcept
        : image_count_(0)
    {
    }

    ArrayIDIDFunc(int preimage_count, int image_count)
        : ArrayIDFunc<int>(preimage_count)
    ...
    void set_image_count(int new_image_count) { image_count_ = new_image_count; }
