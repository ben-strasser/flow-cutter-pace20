...
ArrayIDIDFunc compute_back_arc_permutation(const Tail& tail, const Head& head)
{
    ...
    stable_sort_copy_by_id(std::begin(arc_list), std::end(arc_list),
        std::begin(tmp), node_count,
        [](D d) { return d.head; });
    stable_sort_copy_by_id(std::begin(tmp), std::end(tmp), std::begin(arc_list),
        node_count, [](D d) { return d.tail; });
