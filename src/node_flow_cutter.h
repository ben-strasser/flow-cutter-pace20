...
//!
//! An expanded graph is a virtually expanded version of a symmetric input
//! graph. Expanded means that every original node v is subdivided into two
//! nodes v_in and v_out.
//!
//! For each arc x->y two new inter arcs x_out -> y_in and y_in -> x_out are
//! created For each node x two new intra arcs x_out -> x_in and x_in -> x_out
//! are created Every in to out arc has capacity 1 and every out to in arc has
//! capacity 0.
//!

namespace expanded_graph {
    inline int expanded_node_count(int original_node_count)
    {
        return 2 * original_node_count;
    }
    inline int expanded_arc_count(int original_node_count, int original_arc_count)
    {
        return 2 * (original_node_count + original_arc_count);
    }
    inline int expanded_node_to_original_node(int x) { return x / 2; }
    inline int original_node_to_expanded_node(int x, bool is_out)
    {
        return 2 * x + is_out;
    }
    inline bool get_expanded_node_out_flag(int x) { return x & 1; }
    inline bool is_expanded_intra_arc(int x, int original_arc_count)
    {
        return x >= 2 * original_arc_count;
    }
    inline bool is_expanded_inter_arc(int x, int original_arc_count)
    {
        return x < 2 * original_arc_count;
    }
    inline bool get_expanded_arc_tail_out_flag(int x) { return x & 1; }
    inline int expanded_inter_arc_to_original_arc(int x, int original_arc_count)
    {
        (void)original_arc_count;
        return x / 2;
    }
    inline int original_arc_to_expanded_inter_arc(int x, bool tail_out_flag,
        int original_arc_count)
    {
        return 2 * x + tail_out_flag;
    }
    inline int expanded_intra_arc_to_original_node(int x, int original_arc_count)
    {
        (void)original_arc_count;
        return x / 2 - original_arc_count;
    }
    inline int original_node_to_expanded_intra_arc(int x, bool tail_out_flag,
        int original_arc_count)
    {
        return 2 * (original_arc_count + x) + tail_out_flag;
    }
    ...
    struct Tail {
        ...
        {
            return expanded_arc_count(original_node_count, original_arc_count);
        }
        ...
        int operator()(int a) const
        {
            if (is_expanded_intra_arc(a, original_arc_count)) {
                return original_node_to_expanded_node(
                    expanded_intra_arc_to_original_node(a, original_arc_count),
                    get_expanded_arc_tail_out_flag(a));
            } else {
                return original_node_to_expanded_node(
                    original_tail(
                        expanded_inter_arc_to_original_arc(a, original_arc_count)),
                    get_expanded_arc_tail_out_flag(a));
            }
        }
    };
    ...
    template <class OriginalTail>
    Tail<OriginalTail> tail(int original_node_count, int original_arc_count,
        OriginalTail original_tail)
    {
    ...
    template <class OriginalHead>
    Head<OriginalHead> head(int original_node_count, int original_arc_count,
        OriginalHead original_head)
    {
    ...
    struct BackArc {
        ...
        {
            if (is_expanded_intra_arc(a, original_arc_count)) {
                return original_node_to_expanded_intra_arc(
                    expanded_intra_arc_to_original_node(a, original_arc_count),
                    !get_expanded_arc_tail_out_flag(a), original_arc_count);
            } else {
                return original_arc_to_expanded_inter_arc(
                    original_back_arc(
                        expanded_inter_arc_to_original_arc(a, original_arc_count)),
                    !get_expanded_arc_tail_out_flag(a), original_arc_count);
            }
    ...
    template <class OriginalBackArc>
    BackArc<OriginalBackArc> back_arc(int original_node_count,
        int original_arc_count,
        OriginalBackArc original_back_arc)
    {
        return { original_node_count, original_arc_count,
            std::move(original_back_arc) };
    }
    ...
    struct OutArcIter {
        ...
        typedef typename std::decay<decltype(std::begin(
            std::declval<const OriginalOutArc>()(0)))>::type OriginalOutArcIter;
        ...
        OutArcIter(int intra_arc, bool node_out_flag, OriginalOutArcIter base_iter)
            : intra_arc(intra_arc)
            , node_out_flag(node_out_flag)
            , base_iter(base_iter)
        {
        }
        ...
        int dummy; // To whom ever defined the ackward operator-> semantics: Skrew
            // you!
        ...
        friend bool operator!=(OutArcIter l, OutArcIter r) { return !(l == r); }
    };
    ...
    struct OutArc {
        ...
        int preimage_count() const
        {
            return expanded_node_count(original_node_count);
        }
        ...
        Range<OutArcIter<OriginalOutArc>> operator()(int x) const
        {
            ...
            return Range<OutArcIter<OriginalOutArc>> {
                OutArcIter<OriginalOutArc> {
                    original_node_to_expanded_intra_arc(original_x, out_flag,
                        original_arc_count),
                    out_flag, std::begin(r) },
    ...
    template <class OriginalOutArc>
    OutArc<OriginalOutArc> out_arc(int original_node_count, int original_arc_count,
        OriginalOutArc original_out_arc)
    {
    ...
    Graph<expanded_graph::Tail<Tail>, expanded_graph::Head<Head>,
        expanded_graph::BackArc<BackArc>, expanded_graph::Capacity,
    ...
    make_graph(Tail tail, Head head, BackArc back_arc, OutArc out_arc)
    {
        ...
        return { expanded_graph::tail(node_count, arc_count, std::move(tail)),
    ...
    inline MixedCut
    expanded_cut_to_original_mixed_cut(const std::vector<int>& expanded_cut,
        int original_arc_count)
    {
        ...
        for (auto x : expanded_cut) {
            if (is_expanded_inter_arc(x, original_arc_count)) {
                original_cut.arcs.push_back(
                    expanded_inter_arc_to_original_arc(x, original_arc_count));
            } else {
                original_cut.nodes.push_back(
                    expanded_intra_arc_to_original_node(x, original_arc_count));
            }
        }
    ...
    Separator extract_original_separator(const Tail& tail, const Head& head,
        const FlowCutter& cutter)
    {
    ...
    inline std::vector<SourceTargetPair>
    expand_source_target_pair_list(std::vector<SourceTargetPair> p)
    {
        for (auto& x : p) {
