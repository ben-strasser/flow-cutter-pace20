...
struct ArrayIDMultiFunc {
    ...
    {
        return preimage_to_intermediate.preimage_count();
    }

    Range<T*> operator()(int id)
    {
        assert(0 <= id && id < preimage_count() && "id out of bounds");
        return { intermediate_to_image.begin() + *std::begin(preimage_to_intermediate(id)),
...
//! Inverts an id-id function f. In this context we have two ID types: preimage
//! IDs and image IDs. f maps preimage IDs onto image IDs. This function
//! computes a id-id multi function g that maps image IDs onto preimage ID
//! ranges. g(x) is a range of all y such that f(y) = x ordered increasing by y.
template <class IDIDFunc>
ArrayIDIDMultiFunc invert_id_id_func(const IDIDFunc& f)
{
    ArrayIDIDMultiFunc g = {
        RangeIDIDMultiFunc { ArrayIDFunc<int> { f.image_count() + 1 } },
...
ArrayIDIDMultiFunc compute_successor_function(const Tail& tail,
    const Head& head)
{
