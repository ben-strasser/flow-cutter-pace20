...
struct Config {
    ...
    enum class SkipNonMaximumSides { skip,
        no_skip };
        ...
        if (var == "SkipNonMaximumSides" || var == "skip_non_maximum_sides") {
            if (val == "skip" || val_id == static_cast<int>(SkipNonMaximumSides::skip))
                ...
                throw std::runtime_error(
                    "Unknown config value " + val + " for variable SkipNonMaximumSides; valid are skip, no_skip");
            ...
            if (val == "node_min_expansion" || val_id == static_cast<int>(SeparatorSelection::node_min_expansion))
                ...
                throw std::runtime_error(
                    "Unknown config value " + val + " for variable SeparatorSelection; valid are node_min_expansion, "
                                                    "edge_min_expansion, node_first, edge_first");
            ...
            if (val == "pseudo_depth_first_search" || val_id == static_cast<int>(GraphSearchAlgorithm::pseudo_depth_first_search))
                ...
                throw std::runtime_error("Unknown config value " + val + " for variable GraphSearchAlgorithm; valid "
                                                                         "are pseudo_depth_first_search, "
            ...
            if (val == "avoid_and_pick_best" || val_id == static_cast<int>(AvoidAugmentingPath::avoid_and_pick_best))
                ...
                throw std::runtime_error(
                    "Unknown config value " + val + " for variable AvoidAugmentingPath; valid are avoid_and_pick_best, "
                                                    "do_not_avoid, avoid_and_pick_oldest, avoid_and_pick_random");
            ...
            if (val == "max_target_minus_source_hop_dist" || val_id == static_cast<int>(PierceRating::max_target_minus_source_hop_dist))
                ...
                throw std::runtime_error(
                    "Unknown config value " + val + " for variable PierceRating; valid are "
                                                    "max_target_minus_source_hop_dist, min_source_hop_dist, "
                                                    "max_target_hop_dist, random, oldest");
        } else if (var == "cutter_count") {
            int x = std::stoi(val);
            if (!(x > 0))
                throw std::runtime_error(
                    "Value for \"cutter_count\" must fullfill \"x>0\"");
            cutter_count = x;
            ...
            if (!(x >= 1))
                throw std::runtime_error(
                    "Value for \"max_cut_size\" must fullfill \"x>=1\"");
            max_cut_size = x;
        } else if (var == "min_small_side_size") {
            float x = std::stof(val);
            if (!(0.5 >= x && x >= 0.0))
                throw std::runtime_error("Value for \"min_small_side_size\" must "
                                         "fullfill \"0.5>=x&&x>=0.0\"");
    ...
    {
        if (var == "SkipNonMaximumSides" || var == "skip_non_maximum_sides") {
            if (skip_non_maximum_sides == SkipNonMaximumSides::skip)
                return "skip";
            else if (skip_non_maximum_sides == SkipNonMaximumSides::no_skip)
                return "no_skip";
            else {
                assert(false);
                return "";
            ...
            if (separator_selection == SeparatorSelection::node_min_expansion)
                return "node_min_expansion";
            else if (separator_selection == SeparatorSelection::edge_min_expansion)
                return "edge_min_expansion";
            else if (separator_selection == SeparatorSelection::node_first)
                return "node_first";
            else if (separator_selection == SeparatorSelection::edge_first)
                return "edge_first";
            else {
                assert(false);
                return "";
            ...
            if (graph_search_algorithm == GraphSearchAlgorithm::pseudo_depth_first_search)
                return "pseudo_depth_first_search";
            else if (graph_search_algorithm == GraphSearchAlgorithm::breadth_first_search)
                return "breadth_first_search";
            else if (graph_search_algorithm == GraphSearchAlgorithm::depth_first_search)
            ...
            if (avoid_augmenting_path == AvoidAugmentingPath::avoid_and_pick_best)
                return "avoid_and_pick_best";
            else if (avoid_augmenting_path == AvoidAugmentingPath::do_not_avoid)
                return "do_not_avoid";
            else if (avoid_augmenting_path == AvoidAugmentingPath::avoid_and_pick_oldest)
                return "avoid_and_pick_oldest";
            else if (avoid_augmenting_path == AvoidAugmentingPath::avoid_and_pick_random)
            ...
            if (pierce_rating == PierceRating::max_target_minus_source_hop_dist)
                return "max_target_minus_source_hop_dist";
            else if (pierce_rating == PierceRating::min_source_hop_dist)
                return "min_source_hop_dist";
            else if (pierce_rating == PierceRating::max_target_hop_dist)
                return "max_target_hop_dist";
            else if (pierce_rating == PierceRating::random)
                return "random";
            else if (pierce_rating == PierceRating::oldest)
                return "oldest";
            else {
                assert(false);
                return "";
            ...
            throw std::runtime_error(
                "Unknown config variable " + var + "; valid are "
                                                   "SkipNonMaximumSides,SeparatorSelection,GraphSearchAlgorithm,"
    ...
    std::string get_config() const
    {
        ...
        out << std::setw(30) << "SkipNonMaximumSides"
            << " : " << get("SkipNonMaximumSides") << '\n'
            << std::setw(30) << "SeparatorSelection"
            << " : " << get("SeparatorSelection") << '\n'
            << std::setw(30) << "GraphSearchAlgorithm"
            << " : " << get("GraphSearchAlgorithm") << '\n'
            << std::setw(30) << "AvoidAugmentingPath"
            << " : " << get("AvoidAugmentingPath") << '\n'
            << std::setw(30) << "PierceRating"
            << " : " << get("PierceRating") << '\n'
            << std::setw(30) << "cutter_count"
            << " : " << get("cutter_count") << '\n'
            << std::setw(30) << "random_seed"
            << " : " << get("random_seed") << '\n'
            << std::setw(30) << "max_cut_size"
            << " : " << get("max_cut_size") << '\n'
            << std::setw(30) << "min_small_side_size"
            << " : " << get("min_small_side_size") << '\n';
        return out.str();
    }
};
} // namespace flow_cutter
