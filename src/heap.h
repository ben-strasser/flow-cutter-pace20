...
public:
    ...
    //! Takes a range of std::pair<int, key_type> elements (or any other struct
    //! with first and second) and builds a heap containing these elements.
    ...
    void reset(int new_id_count = 0,
        key_order_type new_order = key_order_type())
    {
...
private:
    ...
    static int children_begin(int pos) { return k * pos + 1; }

    static int children_end(int pos) { return k * (pos + 1) + 1; }
...
public:
    ...
    explicit kway_max_id_heap(int id_count,
        key_order_type order = key_order_type())
    ...
    //! This function has linear running time, whereas repeated push operations
    //! have a total running time in O(n log n)
    template <class Range>
    void fill(const Range& range) { heap.fill(range); }
    ...
    void reset(key_order_type new_order) { heap.reset(std::move(new_order)); }

    bool empty() const { return heap.empty(); }

    int size() const { return heap.size(); }

    bool contains(int id) const { return heap.contains(id); }

    const key_type& get_key(int id) const { return heap.get_key(id); }

    void push(int id, key_type key) { heap.push(id, std::move(key)); }
    ...
    key_type peek_max_key() const { return heap.peek_min_key(); }

    int peek_max_id() const { return heap.peek_min_id(); }

    int pop() { return heap.pop(); }

private:
    struct inverted_order {
        inverted_order() {}
        inverted_order(key_order_type order)
            : order(std::move(order))
        {
        }
...
class min_id_heap
    : public kway_min_id_heap<keyT, standard_heap_arity, key_orderT> {
private:
...
template <class keyT, class key_orderT = std::less<keyT>>
class max_id_heap
    : public kway_max_id_heap<keyT, standard_heap_arity, key_orderT> {
private:
