...
make_graph(Tail tail, Head head, BackArc back_arc, Capacity capacity,
    OutArc out_arc)
{
...
Graph<Tail, Head, BackArc, ConstIntIDFunc<1>, OutArc>
...
public:
    ...
    void grow(
        ...
        const OnNewNode& on_new_node, // on_new_node(x) is called for every node
        // x. If it returns false then the search is
        // stopped, if it returns true it continues
        const ShouldFollowArc& should_follow_arc, // is called for a subset of arcs and must say
        // whether the arc sould be followed
        const OnNewArc& on_new_arc // on_new_arc(xy) is called for ever arc xy
        // with x in the set
    )
    {
        assert(can_grow());
...
public:
    ...
    bool
    does_next_advance_increase_cut(const Graph& graph,
        const ScorePierceNode& score_pierce_node)
    {
        int side = get_current_cut_side();
    ...
    {
        if (reachable[source_side].node_count_inside() == assimilated[source_side].node_count_inside() && (reachable[target_side].node_count_inside() != assimilated[target_side].node_count_inside() || assimilated[source_side].node_count_inside() <= assimilated[target_side].node_count_inside()))
            return source_side;
        else
            return target_side;
    }
...
std::vector<SourceTargetPair>
select_random_source_target_pairs(int node_count, int cutter_count, int seed)
{
