...
template <class Func>
typename std::enable_if<has_int_call_operator<Func>::value,
    LambdaIDFunc<Func>>::type
id_func(int preimage_count, Func func)
{
...
struct LambdaIDIDFunc {
    static_assert(
        std::is_same<int, typename id_func_image_type<IDFunc>::type>::value,
        "IDIDFunc must return int");
...
template <class IDFunc>
typename std::enable_if<is_id_func<IDFunc>::value, LambdaIDIDFunc<IDFunc>>::type
id_id_func(int image_count, IDFunc func)
{
...
template <class Func>
typename std::enable_if<has_int_call_operator<Func>::value,
    LambdaIDIDFunc<LambdaIDFunc<Func>>>::type
id_id_func(int preimage_count, int image_count, Func func)
{
...
struct ConstIntIDFunc {
    explicit ConstIntIDFunc(int preimage_count)
...
public:
    ...
    ConstRefIDIDFunc(const IDIDFunc& x)
        : ptr(&x)
    {
    }
...
public:
    ...
    ConstRefIDFunc(const IDFunc& x)
        : ptr(&x)
    {
    }

    int preimage_count() const { return ptr->preimage_count(); }
    decltype(std::declval<const IDFunc>()(0)) operator()(int x) const
    {
        return (*ptr)(x);
    }
