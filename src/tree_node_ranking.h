...
//! The result is guaranteed to be optimal for trees. For non-trees there are no
//! guarantees.
...
ArrayIDIDFunc compute_tree_node_ranking(const Neighbors& neighbors)
{
    ...
    ArrayIDFunc<int> parent(node_count), child_first_order(node_count);
    ...
    // Compute node levels
    {
        ...
        for (int i = 0; i < node_count; ++i) {
            ...
            if (children_count == 0) {
                ...
                // http://www.sciencedirect.com/science/article/pii/0020019089901610
                int max = 0;
                int p = 0;
                int q = 0;
                ...
                if (q == 0) {
                    ...
                    while (!first_child_critical_list.empty() && first_child_critical_list.back() <= max)
                        first_child_critical_list.pop_back();
                    assert(std::is_sorted(tree_info.critical_list.begin(),
                        tree_info.critical_list.end()));
