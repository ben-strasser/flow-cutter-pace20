...
typename std::enable_if<
    ...
    ArrayIDFunc<typename id_func_image_type<IDFunc>::type>>::type
keep_if(const Pred& p, int new_preimage_count, const IDFunc& f)
{
    assert(p.preimage_count() == f.preimage_count());
    assert(new_preimage_count == count_true(p));

    ArrayIDFunc<typename id_func_image_type<IDFunc>::type> result(
        new_preimage_count);
...
template <class Pred, class IDFunc>
typename std::enable_if<is_id_id_func<IDFunc>::value, ArrayIDIDFunc>::type
keep_if(const Pred& p, int new_preimage_count, const IDFunc& f)
{
    assert(p.preimage_count() == f.preimage_count());
    assert(new_preimage_count == count_true(p));
...
template <class Pred>
ArrayIDIDFunc compute_inverse_keep_function(const Pred& pred,
    int new_image_count)
{
