...
public:
    static_assert(1 <= bit_count && bit_count <= 64,
        "an integer with more than 64 bits is not tiny");
    ...
    int preimage_count() const { return preimage_; }

    template <class IDFunc>
    TinyIntIDFunc(
        const IDFunc& other,
        typename std::enable_if<is_id_func<IDFunc>::value, void>::type* dummy = 0)
        : preimage_(other.preimage_count())
        , data_((other.preimage_count() + entry_count_per_uint64 - 1) / entry_count_per_uint64)
    {
    ...
    std::uint64_t move(int id) { return operator()(id); }
    ...
    template <class IDFunc>
    TinyIntIDFunc
    operator=(const typename std::enable_if<is_id_func<IDFunc>::value,
        IDFunc>::type& other)
    {
